...
const errorHandler = (error: ResponseError) => {
  ...
  if (!skipErrorHandler && response && response.status) {
...
const getToken = () => {
  const v = JSON.parse(localStorage.getItem(KEYS.TOKEN) || '{}') as API.LoginResult;
  return v;
};
...
export const request: RequestConfig = {
  ...
  middlewares: [
    async function middlewareA(ctx, next) {
      ...
      if (!ctx.req.url.startsWith('http')) ctx.req.url = GLOBAL_CONFIG.url + ctx.req.url;
      ...
      if (token) {
        // @ts-ignore
        ctx.req.options.headers.authorization = `Bearer ${token.access_token}`;
      }
      // console.log('A before');
      await next();

      // console.log(ctx);
      if (ctx.res && !ctx.res.detailsErrorMessage) {
